    ...
    <nav className="bg-white border-b border-navy-100 sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex items-center gap-8">
            ...
            <Link to="/login" className="text-sm font-medium text-navy-600 hover:text-navy-900 transition">Get Started</Link>
      ...
      {open && (
        <div className="md:hidden bg-white border-t border-navy-100 px-4 pb-4 space-y-2">
          ...
          <Link to="/login" className="block py-2 text-sm font-medium text-navy-700" onClick={() => setOpen(false)}>Get Started</Link>
          <Link to="/login" className="block w-full text-center px-5 py-2.5 bg-navy-900 text-white text-sm font-semibold rounded-xl" onClick={() => setOpen(false)}>Get Started</Link>
        </div>
      )}
    </nav>
  );
