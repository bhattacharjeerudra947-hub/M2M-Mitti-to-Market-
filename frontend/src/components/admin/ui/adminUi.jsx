...
export function StatusDot({ tone = 'gray', label }) {
  return (
    <span className="inline-flex items-center gap-1.5 whitespace-nowrap">
      <span className={`h-1.5 w-1.5 rounded-full ${STATUS_TONES[tone] || STATUS_TONES.gray}`} />
...
export function verificationStatusInfo(u) {
  if (u?.verified || u?.verificationStatus === 'VERIFIED') return { tone: 'green', label: 'Verified' };
  const s = (u?.verificationStatus || '').toUpperCase();
  if (s === 'PENDING' || s === 'DOCUMENTS_SUBMITTED' || s === 'UNDER_REVIEW') return { tone: 'amber', label: 'Pending' };
  if (s === 'REJECTED') return { tone: 'red', label: 'Rejected' };
  if (s === 'RE_SUBMISSION_REQUESTED' || s === 'RESUBMISSION_REQUIRED') return { tone: 'blue', label: 'Resubmission requested' };
...
export function accountStatusInfo(status) {
  switch ((status || 'ACTIVE').toUpperCase()) {
    ...
    case 'SUSPENDED': return { tone: 'amber', label: 'Suspended' };
    case 'DEACTIVATED': return { tone: 'red', label: 'Deactivated' };
    default: return { tone: 'gray', label: status || 'Unknown' };
  }
}

export function genericStatusInfo(status) {
  const s = (status || '').toUpperCase();
  if (['OPEN', 'NEW', 'PENDING', 'LOCK_PENDING', 'NEGOTIATING', 'AVAILABLE'].includes(s)) return { tone: 'amber', label: s };
  if (['UNDER_REVIEW', 'REVIEWING', 'IN_PROGRESS', 'LOCKED', 'LOGISTICS_PENDING', 'LOGISTICS_ASSIGNED', 'PICKUP_SCHEDULED', 'PICKED_UP', 'IN_TRANSIT', 'OUT_FOR_DELIVERY', 'PARTIALLY_SOLD', 'PARTIALLY_FULFILLED', 'LOW_STOCK', 'ESCALATED'].includes(s)) return { tone: 'blue', label: s };
  if (['RESOLVED', 'COMPLETED', 'VERIFIED', 'DELIVERED', 'DELIVERY_CONFIRMED'].includes(s)) return { tone: 'green', label: s };
  if (['DISMISSED', 'CANCELLED', 'REJECTED', 'SUSPENDED', 'REMOVED', 'DEACTIVATED'].includes(s)) return { tone: 'red', label: s };
  return { tone: 'gray', label: s || '—' };
}
...
export function Metric({ label, value, context, onClick }) {
  ...
  return (
    <Comp
      onClick={onClick}
      className={`text-left px-4 py-3 ${onClick ? 'hover:bg-gray-50 transition-colors w-full' : ''} ${onClick ? 'cursor-pointer' : ''}`}
    >
      <p className="text-[11px] font-medium uppercase tracking-wide text-gray-500">{label}</p>
      <p className="mt-0.5 text-2xl leading-7 font-semibold text-gray-900 tabular-nums">{value ?? '—'}</p>
      {context && <p className="mt-0.5 text-xs text-gray-500">{context}</p>}
    </Comp>
  );
}
          ...
          <thead>
            <tr className="border-b border-gray-200 bg-gray-50/60">
              {columns.map((c) => (
                <th
                  key={c.key}
                  className={`px-4 py-2.5 text-[11px] font-semibold uppercase tracking-wide text-gray-500 whitespace-nowrap ${c.align === 'right' ? 'text-right' : ''}`}
                  style={{ width: c.width }}
                >
                  {c.label}
                </th>
...
export function Td({ children, className = '', colSpan }) {
  return <td colSpan={colSpan} className={`px-4 py-3 text-[13px] text-gray-700 align-middle ${className}`}>{children}</td>;
}

/* ── Skeleton rows (loading) — renders <tr>s inside the Table's tbody ── */
        ...
        <tr key={r}>
          {Array.from({ length: cols }).map((__, c) => (
            <td key={c} className="px-4 py-3.5">
              <div className="h-3.5 rounded bg-gray-100 animate-pulse" style={{ width: c === 0 ? '60%' : `${45 + ((r * 7 + c * 13) % 35)}%` }} />
            </td>
          ))}
        </tr>
      ))}
...
/* ── Empty state ──────────────────────────────────────────────── */
export function EmptyState({ title = 'No results found', hint }) {
  return (
    <div className="flex flex-col items-center justify-center py-16 px-6 text-center">
      <Inbox className="h-8 w-8 text-gray-300 mb-3" strokeWidth={1.5} />
...
export function ErrorState({ message = 'Unable to load data', onRetry }) {
  return (
    <div className="flex flex-col items-center justify-center py-16 px-6 text-center">
      <AlertCircle className="h-8 w-8 text-red-300 mb-3" strokeWidth={1.5} />
...
/* ── Confirmation dialog ──────────────────────────────────────── */
export function ConfirmDialog({ open, title, body, confirmLabel = 'Confirm', destructive = false, requireReason = false, reasonPlaceholder, busy, error, onCancel, onConfirm, onReasonChange, reason }) {
  useEffect(() => {
    if (!open) return;
    ...
    <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-gray-950/40" role="dialog" aria-modal="true">
      <div className="bg-white w-full max-w-md rounded-xl border border-gray-200 shadow-xl">
        <div className="px-6 pt-5 pb-1">
          ...
          {body && <p className="mt-1.5 text-[13px] leading-relaxed text-gray-600">{body}</p>}
        </div>

        {error && (
          <div className="mx-6 mt-3 px-3 py-2 rounded-lg bg-red-50 border border-red-100 text-[13px] text-red-700">{error}</div>
        )}
        ...
        {requireReason && (
          <div className="px-6 mt-4">
            ...
            <textarea
              ...
              rows={3}
              placeholder={reasonPlaceholder || 'Document the reason for this action…'}
        ...
        <div className="flex items-center justify-end gap-2 px-6 py-4">
          ...
          <button
            onClick={onConfirm}
            disabled={busy || (requireReason && (!reason || !reason.trim()))}
...
export function Drawer({ open, onClose, title, children, footer, width = 'max-w-xl' }) {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex justify-end">
      ...
      <div className={`relative bg-white h-full w-full ${width} border-l border-gray-200 shadow-2xl flex flex-col`}>
        <div className="flex items-center justify-between px-6 h-14 border-b border-gray-200 shrink-0">
          <h3 className="text-[15px] font-semibold text-gray-900">{title}</h3>
          <button onClick={onClose} className="p-1.5 -mr-1.5 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-md" aria-label="Close">
            <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M4 4l8 8M12 4l-8 8" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" /></svg>
          </button>
        </div>
        <div className="flex-1 overflow-y-auto px-6 py-5">{children}</div>
        {footer && <div className="border-t border-gray-200 px-6 py-3.5 shrink-0">{footer}</div>}
      </div>
    </div>
  );
}
...
export function DrawerSection({ title, children }) {
  return (
    <section className="mb-6">
      <h4 className="text-[11px] font-semibold uppercase tracking-wide text-gray-500 mb-2.5">{title}</h4>
      {children}
    </section>
  );
}
...
export function KV({ k, v }) {
  return (
    <div className="flex justify-between gap-4 py-1.5 border-b border-gray-50 last:border-0">
      ...
      <span className="text-[13px] text-gray-900 text-right font-medium break-all">{v ?? '—'}</span>
    </div>
  );
}
...
export function RowActions({ items }) {
  // items: [{label, onClick, destructive}]; first visible item is primary, rest go in ⋯ menu
              ...
              {rest.map((it) => (
                <button
                  ...
                  className={`w-full text-left px-3.5 py-2 text-[13px] hover:bg-gray-50 transition-colors ${
                    it.destructive ? 'text-red-600' : 'text-gray-700'
                  }`}
                >
                  {it.label}
                </button>
