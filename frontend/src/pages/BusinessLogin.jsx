...
/**
 * Business/Buyer Login Portal — only accepts BUSINESS accounts.
...
export default function BusinessLogin() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    const result = await login(email.trim(), password, 'business');
    setLoading(false);
        ...
        <div className="max-w-md mx-auto">
          <div className="text-center mb-6">
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-mustard-50 rounded-full text-sm font-medium text-navy-800 border border-mustard-200 mb-4">
              <span className="text-xl">🏪</span> Business / Buyer Login Portal
            </div>
            <h1 className="text-3xl font-bold text-gray-900 mb-2">Sign In — Business / Buyer</h1>
            <p className="text-gray-500">Only Business/Buyer accounts can sign in here</p>
          </div>
