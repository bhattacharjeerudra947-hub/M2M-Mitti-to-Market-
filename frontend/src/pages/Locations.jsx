...
function makeIcon(color) {
  return L.divIcon({
    ...
    html: `<div style="position:relative;width:22px;height:22px">
      <div style="width:22px;height:22px;border-radius:50%;background:${color};border:2.5px solid white;box-shadow:0 1px 4px rgba(0,0,0,.4);display:flex;align-items:center;justify-content:center;font-size:11px;line-height:1"></div>
    </div>`,
    iconSize: [22, 22],
    iconAnchor: [11, 11],
    popupAnchor: [0, -12],
...
export default function Locations() {
  ...
  return (
    <div className="flex min-h-screen bg-mustard-50/30">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 mb-5">
            <div>
              <h1 className="text-2xl font-bold text-navy-900 flex items-center gap-2">
                <MapPin className="w-6 h-6 text-mustard-600" /> Locations
              </h1>
              ...
              <div className="lg:col-span-3">
                <div className="bg-white rounded-2xl border border-navy-100 shadow-sm overflow-hidden">
                    ...
                    <MapContainer center={center} zoom={5} scrollWheelZoom={false} style={{ height: '520px', width: '100%' }}>
                      ...
                      {filtered.filter(l => l.latitude != null && l.longitude != null).map((l) => (
                        <Marker key={l.id} position={[l.latitude, l.longitude]}
                          icon={makeIcon(l.role === 'FARMER' ? FARMER_COLOR : BUSINESS_COLOR)}>
                          <Popup>
                            <div className="min-w-[180px]">
                              ...
                              <p className="text-[11px] text-gray-400 mt-1">
                                {l.locationAccuracy === 'EXACT' ? 'Shared location' : 'Approximate city location'}
                              </p>
                            </div>
                          </Popup>
                        </Marker>
                  ...
                  <div className="flex items-center gap-4 px-4 py-2 bg-gray-50 border-t border-gray-100 text-[11px] text-gray-500">
                    ...
                    <span className="ml-auto text-[10px] text-gray-400">OpenStreetMap</span>
                  </div>
                </div>
              </div>
              ...
              <div className="lg:col-span-2 space-y-4">
                <div className="bg-white rounded-2xl border border-navy-100 shadow-sm p-4">
                  <p className="text-xs text-gray-400 uppercase font-medium tracking-wide mb-3 flex items-center gap-1.5">
                    <Users className="w-3.5 h-3.5 text-emerald-500" /> Farmers ({farmers.length})
                  </p>
                ...
                <div className="bg-white rounded-2xl border border-navy-100 shadow-sm p-4">
                  <p className="text-xs text-gray-400 uppercase font-medium tracking-wide mb-3 flex items-center gap-1.5">
                    <Store className="w-3.5 h-3.5 text-blue-500" /> Businesses ({businesses.length})
                  </p>
      ...
      {/* (marker pulse animation removed with live tracking) */}
    </div>
  );
}
...
function LocationRow({ l }) {
  return (
    <div className="p-2.5 rounded-xl border text-sm bg-gray-50 border-gray-100">
      <div className="flex items-center justify-between gap-2">
        <p className="font-semibold text-navy-900 truncate">
          {l.role === 'FARMER' ? '👨🌾' : '🏪'} {l.name}
        </p>
        <span className={`text-[10px] px-1.5 py-0.5 rounded-full ${l.verified ? 'text-emerald-600 bg-emerald-100 font-bold' : 'text-gray-400'}`}>
          {l.verified ? '✓ Verified' : ''}
        </span>
      </div>
