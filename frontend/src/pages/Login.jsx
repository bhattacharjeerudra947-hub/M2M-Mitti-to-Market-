...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    // For now, the api.login expects email; if phone login, pass phone as email placeholder
    // The backend /api/auth/login only accepts email+password
    const loginEmail = loginMethod === 'email' ? email : email;
    const result = await login(loginEmail || phone, password);
    setLoading(false);
